...
const ProductDetails = () => {
        ...
        <>
          {isProductNotFound ? (
            ...
            <>
              <Helmet>
                {/* Dynamic Title */}
                <title>{`${product.name} | ${product.category} | Farmers help`}</title>
                ...
                <meta
                  name="keywords"
                  content={`buy ${product.name}, ${product.category}, ${product.name} online, ${product.category} products, Farmers Help shopping`}
                />

                {/* Open Graph for Social Sharing */}
                <meta
                  property="og:title"
                  content={`${product.name} | Farmers Help`}
                />
  ...
  <div className="relative">
    <div className="relative rounded-2xl overflow-hidden shadow-lg border border-gray-200 bg-white">
      ...
      <button
        onClick={prevImage}
        className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 p-2 rounded-full hover:bg-white transition-colors duration-300 shadow"
      >
        <ChevronLeft className="h-6 w-6 text-gray-700" />
      </button>
      <button
        onClick={nextImage}
        className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 p-2 rounded-full hover:bg-white transition-colors duration-300 shadow"
      >
    ...
    <div className="flex space-x-4 mt-4 overflow-x-auto pb-2">
      {product?.images?.map((image, index) => (
        <button
          ...
          className={`relative h-16 w-16 md:h-20 md:w-20 flex-shrink-0 rounded-lg overflow-hidden border ${
            currentImage === index ? "ring-2 ring-orange-500" : "border-gray-200"
          }`}
        >
          <img
            src={image.url}
      ...
      <div className="flex items-center space-x-3 md:space-x-4">
        ...
        {product.discount > 0 && (
          <>
            ...
            <span className="bg-rose-100 text-rose-700 px-2 py-1 rounded-md text-xs md:text-sm font-semibold">
              {product.discount}% OFF
            </span>
          </>
        )}
      </div>
      <p className="text-sm md:text-base text-red-500 mt-2 font-medium">
        Only {product.inStock} items left!
      </p>
    </div>

    <div className="mb-4 md:mb-6">
      <h3 className="text-lg font-semibold mb-2 md:mb-3 text-gray-800">
        Description
      </h3>
        ...
        <div className="flex flex-wrap gap-2 md:gap-3">
          {product.availableSize?.split(",").map((size) => (
            <button
              ...
              className={`px-3 md:px-4 py-1.5 md:py-2 rounded-lg border text-sm md:text-base font-medium ${
                selectedSize === size
    ...
    <div className="mb-6 md:mb-8">
      ...
      <ul className="space-y-1 md:space-y-2">
        {product.features?.split(",").map((feature, index) => (
          ...
          >
            <Shield className="h-4 w-4 md:h-5 md:w-5 text-lime-500 mr-2" />
            {feature}
          </li>
        ))}
      </ul>
    ...
    <div className="space-y-3 md:space-y-4">
      {doesProductExist ? (
        <button
          ...
          className="w-full bg-rose-600 text-white py-2 md:py-3 rounded-xl font-semibold hover:bg-rose-700 transition duration-300 flex items-center justify-center gap-2"
        >
        ...
        <button
          disabled={isAdding}
          onClick={addToCart}
          className="w-full bg-emerald-600 text-white py-2 md:py-3 rounded-xl font-semibold hover:bg-emerald-700 transition duration-300"
        >
          {isAdding ? "Adding..." : "Add to Cart"}
        </button>
      )}

      {/* <button className="w-full bg-gray-900 text-white py-2 md:py-3 rounded-xl font-semibold hover:bg-gray-800 transition duration-300">
        Buy Now
      </button> */}
    </div>

    <div className="mt-6 md:mt-8 grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4">
      <div className="flex items-center justify-center p-3 md:p-4 bg-orange-50 border border-orange-100 rounded-xl text-xs md:text-sm font-medium text-orange-800">
      ...
      <div className="flex items-center justify-center p-3 md:p-4 bg-orange-50 border border-orange-100 rounded-xl text-xs md:text-sm font-medium text-orange-800">
        ...
        Quality Product
                ...
                <div className="mt-16">
                  ...
                  {/* Write a Review */}
                  {!isLoggedIn ? (
                    <div className="bg-gradient-to-bl from-green-50 to-lime-50 border border-gray-200 rounded-xl p-6 shadow-sm text-center max-w-xl mx-auto mt-10">
                      ...
                      <div className="flex justify-center gap-4">
                        ...
                        <button
                          ...
                          className="px-6 py-2 rounded-lg border border-lime-600 text-lime-600 hover:bg-blue-50 transition"
                        >
                          Sign Up
                        </button>
                      </div>
