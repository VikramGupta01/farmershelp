...
const Profile = () => {
    ...
    <div className="max-w-7xl min-h-[90vh] mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          <div className="bg-gradient-to-b from-emerald-50 to-lime-50 rounded-xl shadow-sm p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleDetailsInputChange}
                      disabled={true}
                      className="w-full px-4 py-2 border bg-slate-200 text-slate-500 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
  ...
  {userOrders.length === 0 ? (
    <div className="text-center py-12 px-6 bg-white rounded-2xl shadow-md border border-dashed border-gray-300">
      <div className="flex justify-center mb-4">
        <Package className="h-14 w-14 text-gray-400" />
      </div>
      <h4 className="text-lg font-semibold text-gray-700 mb-2">No Orders Yet</h4>
      <p className="text-sm text-gray-500 mb-4">You haven't placed any orders. When you do, they’ll appear here.</p>
      <Link
        to="/"
        className="inline-block bg-emerald-600 text-white px-6 py-2 rounded-full text-sm font-medium hover:bg-emerald-700 transition-all duration-200"
      >
        Start Shopping
      </Link>
    </div>
    ...
    <div className="space-y-6">
      {userOrders?.map((order) => (
        <div
          key={order.orderId}
          className="bg-gradient-to-b from-lime-50 to-emerald-100 border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow"
        >
          ...
          <div className="text-sm text-gray-600 space-y-1">
            ...
            <p>Items: <span className="font-medium text-gray-700">{order.products.length}</span></p>
            <p>Total: <span className="font-medium text-gray-700">₹{order.finalPrice}</span></p>
          </div>

          <Link
            to={`/order/${order.orderId}`}
            className="mt-4 inline-flex items-center text-emerald-600 hover:text-emerald-700 text-sm font-medium transition-colors"
          >
            View Details
