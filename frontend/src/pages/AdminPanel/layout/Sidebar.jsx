...
const Sidebar = ({ onClose,onLogout }) => {
  ...
  return (
    <aside className="w-64 bg-lime-50 shadow-xl h-screen flex flex-col border-r border-gray-200">
      {/* Logo & Close Button */}
      ...
      <nav className="mt-4 flex flex-col gap-1 px-4">
        {navItems.map((item) => (
          <NavLink
            ...
            onClick={onClose}
            className={({ isActive }) =>
              `flex items-center px-4 py-3 rounded-lg text-gray-700 transition duration-200 hover:bg-lime-100 hover:text-emerald-600 ${
                isActive ? "bg-lime-100 text-gray-600 font-semibold" : ""
              }`
            }
          >
