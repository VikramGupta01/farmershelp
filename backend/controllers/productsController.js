    ...
    try {
      ...
      const imageUploadPromises = req.files.map((file)=>{
        return new Promise((resolve,reject)=>{
          const uploadstream = cloudinary.uploader.upload_stream({folder:"FarmersHelp/Products/", secure:true},
            (error,results)=>{
              if(error)
...
export async function updateProduct(req,res,next){
    try {
      ...
      // Destructure with default empty array for imageUrls
      ...
      // Add safety checks to ensure imageUrls is an array
      const safeImageUrls = Array.isArray(imageUrls) ? imageUrls : []
      
      const imagesToBeDeleted = allProductImages.filter(img => !safeImageUrls.includes(img.url))
      const imagesToBeStayed = allProductImages.filter(img => safeImageUrls.includes(img.url))
      const imagesToBeUploaded = req.files || [] // Add safety check for req.files
        ...
        const uploadImagePromise = imagesToBeUploaded.map(img => {
          return new Promise((resolve,reject) => {
            const uploadstream = cloudinary.uploader.upload_stream(
              {folder:"FarmersHelp/Products/", secure:true},
              (err,results) => {
                if(err)
