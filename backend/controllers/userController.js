...
export async function updateMyPhoto(req,res,next){
  try {
    ...
    const imagePromise = new Promise((resolve,reject)=>{
      const uploadstream = cloudinary.uploader.upload_stream({folder:"FarmersHelp/Users/", secure:true},(error, results)=>{
        if(error)
...
export async function deleteMyPhoto(req,res,next){
  try {
    ...
    const user = await User.findByIdAndUpdate(req.user._id,{
      profileImage:{
        url:"https://res.cloudinary.com/dyzc15a8e/image/upload/v1749206475/uu_umrsod.jpg",
        public_id:"FarmersHelp/Users/default_profile_image"
      }
    },{new:true})
    res.status(200).json({
      status:"success",
